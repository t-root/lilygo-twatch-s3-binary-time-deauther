...
static void updateDeauthUI() {
    ...
    for (size_t i = 0; i < apList.size(); i++) {
        ...
        lv_checkbox_set_text(chk, apList[i].ssid.c_str());
        lv_obj_set_user_data(chk, (void*)i);
        lv_obj_add_event_cb(chk, [](lv_event_t *e) {
            lv_obj_t *obj = (lv_obj_t*)lv_event_get_target(e); // ép kiểu
            size_t idx = (size_t)lv_obj_get_user_data(obj);
            if (idx < apList.size()) apList[idx].selected = !apList[idx].selected;
        }, LV_EVENT_CLICKED, NULL);
        deauthCheckboxes.push_back(chk);
    }

    if (apList.empty()) {
        lv_label_set_text(deauthStatusLabel, "No AP found. Tap Rescan.");
    } else {
        char buf[64];
        snprintf(buf, sizeof(buf), "Found %d APs", (int)apList.size());
        lv_label_set_text(deauthStatusLabel, buf);
    }
}
...
static void deauthSelected() {
    ...
    for (auto &ap : apList) {
        if (ap.selected) {
            sendDeauthPacket(ap.bssid, ap.channel);
            delay(10);
        }
    }
    lv_label_set_text(deauthStatusLabel, "Deauth sent to selected APs");
}

static void selectAll() {
    for (auto &ap : apList) ap.selected = true;
    for (size_t i = 0; i < deauthCheckboxes.size() && i < apList.size(); i++) {
        setCheckboxChecked(deauthCheckboxes[i], true);
    }
    lv_label_set_text(deauthStatusLabel, "All selected");
}

static void deselectAll() {
    for (auto &ap : apList) ap.selected = false;
    for (size_t i = 0; i < deauthCheckboxes.size() && i < apList.size(); i++) {
        setCheckboxChecked(deauthCheckboxes[i], false);
    }
    lv_label_set_text(deauthStatusLabel, "All deselected");
}
...
static void deauth_btn_event_cb(lv_event_t *e) {
    ...
    switch (action) {
        case 0: scanWifi(); updateDeauthUI(); break;
        case 1: deauthSelected(); break;
        case 2: selectAll(); break;
        case 3: deselectAll(); break;
        case 4: exitDeauthMode(); break;
        default: break;
    }
}
...
static void touch_event_cb(lv_event_t *e) {
    ...
    if (code == LV_EVENT_PRESSING) {
        if (touchPressed && pressStartMs != 0) {
            ...
            // 5s -> setting mode
            if (elapsed >= 5000 && (now - lastHoldTriggerMs > 800)) {
                if (!deauthMode) {
                    toggleSettingMode();
                    lastHoldTriggerMs = now;
                    pressStartMs = 0;
...
void setup() {
    ...
    for (int i = 0; i < 10; ++i) {
        ...
        lv_label_set_text(batteryChars[i], "");
        lv_obj_set_style_text_color(batteryChars[i], lv_color_hex(0x00FF66), 0);
    ...
    lv_obj_set_style_border_color(settingPanel, lv_color_hex(0x00FF66), 0);
    ...
    for (uint8_t i = 0; i < 5; ++i) {
        ...
        lv_obj_set_style_text_color(txt, lv_color_hex(0x00FF66), 0);
        lv_obj_center(txt);
        addTouchBubble(txt);
    }
    lv_obj_add_flag(settingPanel, LV_OBJ_FLAG_HIDDEN);
    ...
    lv_obj_set_style_text_color(deauthStatusLabel, lv_color_hex(0x00FF66), 0);
    ...
    const char *deauthBtnText[] = {"Rescan", "Deauth", "Sel All", "Desel All", "Exit"};
    int deauthBtnAction[] = {0, 1, 2, 3, 4};
    for (int i = 0; i < 5; i++) {
...
void loop() {
    ...
    if (screenOn && (now - lastMotionMs > 10000)) {
        sleepScreen();
    }
